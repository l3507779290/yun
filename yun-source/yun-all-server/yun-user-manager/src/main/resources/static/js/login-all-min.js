...
var DOWNLOAD_DES = [["大小：5.78MB", "版本：PC版 V2.0.0", "适应系统：windows XP/vista/win7/win8", "更新时间：2013-02-25"], ["大小：1.6M", "版本：Android版3.0.0", "适应系统：Android2.2、Android2.3<br/><i>Android4.0</i>", "更新时间：2012/6/13"], ["大小：2.5M", "版本：Iphone版2.0.0", "适应系统：IOS 4.0 及以上<br/><i>（需越狱后才能安装）</i>", "更新时间：2012/6/7"], ["大小：5.6M", "版本：iPad版 V2.0.0", "适应系统：IOS 4.3 及以上<br/><i>（需越狱后才能安装）</i>", "更新时间：2012/8/15"], ["大小：5M", "版本：WinPhone版v2.0.0", "适应系统：Windows Phone 7.5 及以上", "更新时间：2013/03/29"], ["大小：5.51MB", "版本：Window版3.0.0", "适应系统：windows XP/vista/win7", "更新时间：2012/6/19"], ["大小：5M", "版本：Mac版 V2.0.0", "适应系统：Mac OS 10.7 及以上", "更新时间：2013/04/25"]];
var DOWNLOAD_UPDATE = [false, false, false, false, false, false, false];
var DOWNLOAD_LIST = ["windows", "android", "iphone", "ipad", "wphone", "tongbupan", "linux"];
var CO_APK = [{
    src: "http://s1.pan.bdstatic.com/static/images/plug/fex48.png?r=202009155522",
    width: "56px",
    title: "ES文件浏览器百度云特别版",
    desc: "一键备份，简单方便，有条有理，永不丢失。",
    level: 5,
    url: "http://bs.baidu.com/appstore/apk_c5a8e927c1768f0778ede7d4b40ee535.apk"
}, {
    src: "http://s1.pan.bdstatic.com/static/images/plug/icon.png?r=202009155522",
    title: "文件大师",
    width: "56px",
    desc: "最佳Android资源管理文件同步解决方案。",
    level: 5,
    url: "http://www.xageek.com/software/FileExpertLatest.apk"
}, {
    src: "http://s1.pan.bdstatic.com/static/images/plug/icon72.png?r=202009155522",
    width: "54px",
    title: "云舟浏览器",
    desc: "云舟浏览器，让上网变得简洁快速。",
    level: 5,
...
}, {className: "baidu.fx.Timeline", options: {interval: 16, duration: 500, dynamic: true}}).extend({
    launch: function () {
        var c = this;
...
baidu.dom.ready(function () {
    ...
    $.ajax({
        url: "/disk/cmsdata?do=client&t=" + new Date().getTime(),
        type: "GET",
        timeout: 10 * 1000,
        success: function (g, f) {
            ...
            if (o != null && o.errorno == 0) {
                if (o.android) {
                    var l = o.android;
                    DOWNLOAD_DES[0] = ["大小：" + l.size, "版本：" + l.version, "适应系统：" + l.system, "更新时间：" + l.publish.split(" ")[0]];
                    DOWNLOAD_URL_LOGIN[0] = "http://bcscdn.baidu.com/netdisk/BaiduYun_" + l.version.match(/V(.+$)/)[1] + ".apk"
                }
                if (o.iphone) {
                    var j = a.iData = o.iphone;
                    DOWNLOAD_DES[1] = ["大小：" + j.size, "版本：" + j.version, "适应系统：" + j.system, "更新时间：" + j.publish.split(" ")[0]]
                }
                if (o.ipad) {
                    var k = o.ipad;
                    DOWNLOAD_DES[2] = ["大小：" + k.size, "版本：" + k.version, "适应系统：" + k.system, "更新时间：" + k.publish.split(" ")[0]]
                }
                if (o.winphone) {
                    var n = o.winphone;
                    DOWNLOAD_DES[3] = ["大小：" + n.size, "版本：" + n.version, "适应系统：" + n.system, "更新时间：" + n.publish.split(" ")[0]]
                }
                if (o.windows) {
                    var m = o.windows;
                    DOWNLOAD_DES[4] = ["大小：" + m.size, "版本：" + m.version.match(/V(.+$)/)[0], "适应系统：" + m.system, "更新时间：" + m.publish.split(" ")[0]];
...
$(window).ready(function () {
    ...
    var d = function () {
        ...
        $.get(j.api.RestAPI.CMS_DATA_SIMPLE + "?do=piece&ad=" + h + "&t=" + (+new Date()), {}, function (o) {
            var q, n, m;
            try {
                q = $.parseJSON(o)
            } catch (p) {
        ...
        if (u) {
            if (u === "hao123") {
                $(".yun-logo").css({
                    ...
                    width: 113,
                    "margin-left": 0
                }).attr({title: "百度网盘", href: "http://yun.baidu.com/", target: "_blank"}).parent().css({
                    width: 250,
                    background: "none",
                    filter: "none"
                })
...
(function (f, h) {
    function c() {
        ...
        var n = /(safari)\/[\d.]+/;
        var k = /version\/([\d.]+)/;
        var m = {broswer: "", version: ""};
        if (l.indexOf("safari") > 0 && l.indexOf("chrome") < 0) {
            m.broswer = (l.match(n) || [])[1];
            m.version = (l.match(k) || [])[1]
        }
        return m
    }
    ...
    function d() {
        var k = c();
        if (k.broswer === "safari" && k.version === "10.1.2") {
            return
        }
        f("#login-container").interactiveBg()
    }
    ...
    var g = (function () {
        if (navigator.appName === "Microsoft Internet Explorer") {
            ...
            var l = m.split(";");
            var k = l[1].replace(/[ ]/g, "").replace("MSIE", "");
            return Number(k)
        } else {
            return 0
        }
    ...
    function j() {
        return (function () {
            var k = setTimeout(function () {
                f("#TANGRAM__PSP_4__QrcodeRefreshBtn").click();
                clearTimeout(k)
            }, 300)
        })()
    }
    ...
    function a(k) {
        if (g && g <= 10) {
            f("#tongbupan-download,#windows-download").on("click", function () {
...
var loginSdkV4 = {
    init: function () {
        ...
        var c = $("#TANGRAM__PSP_4__qrcode .pass-form-logo");
        var o = $("#TANGRAM__PSP_4__form .pass-form-logo");
        var m = $(".tang-pass-footerBar .tang-pass-footerBarQrcode");
        var h = $(".tang-pass-footerBar .tang-pass-footerBarULogin");
        var d = $("#TANGRAM__PSP_4__qrcode .tang-pass-qrcode-title a");
        var j = $("#TANGRAM__PSP_4__qrcode .tang-pass-qrcode-title");
        var g = $("#TANGRAM__PSP_4__smsSwitchWrapper");
        var k = $("#TANGRAM__PSP_4__sms_btn_back");
        var l = $("#TANGRAM__PSP_4__footerULoginBtn");
        var f = $("#TANGRAM__PSP_4__QrcodeAnimation");
        if ($("#TANGRAM__PSP_4__qrcode").find(".tips").length === 0) {
            $("#TANGRAM__PSP_4__qrcode").append('<p class="tips">最新版百度网盘App - 首页右上角 - 扫一扫</p>')
        }
        this.setValue(c, "扫一扫登录");
        this.setValue(m, "扫一扫登录");
        this.setValue(o, "帐号密码登录");
        this.setValue(h, "帐号密码登录");
        j.html('请使用<a href="https://pan.baidu.com/download?from=login#pan" target="_blank">百度网盘App</a>扫码登录');
        g.html(g.html() + "&gt;");
        k.html("帐号密码登录&gt;");
        var a = f.css("backgroundImage");
        var n = a.match(/url\("?(.*)"?\)/)[1];
        f.html('<img style="width:100%;height:auto;" src="' + n + '">');
        f.css("backgroundImage", "none")
    }, getLoginToUrl: function () {
        var f = /redirecturl=(.*)(&|#)?/;
        var g = top ? top : window;
        var d = decodeURIComponent(g.location.search);
        var c = d.match(f);
        var a = location.protocol + "//" + location.host + "/disk/home" + g.location.search + g.location.hash;
        if (c !== null && /baidu\.com/.test(c[1])) {
            a = c[1]
        }
        return a
    }, imgLog: function (c, f) {
        for (var d in f) {
            c += (c.match(/\?/) ? "&" : "?") + encodeURIComponent(d) + "=" + encodeURIComponent("" + f[d])
        }
        c += (c.match(/\?/) ? "&" : "?") + "t=" + (+new Date());
        var a = new Image();
        a.src = c
    }, initPass: function () {
        var a = this;
        $.getScript("https://passport.baidu.com/passApi/js/wrapper.js?cdnversion=" + new Date().getTime(), function () {
            passport.use("login", {tangram: true, loginVersion: "v4"}, function (d) {
                var c = a.getLoginToUrl();
                var f = new d.passport.login({
                    product: "netdisk",
                    loginType: 1,
                    sms: 5,
                    userPwdLogin: 1,
                ...
                f.on("loginSuccess", function (g) {
                    ...
                    try {
                        a.imgLog("//pan.baidu.com/api/analytics", {
                            type: "web_header_title_web_login_success",
                            clienttype: 0,
                            version: "v5",
...
window.onload = function () {
    loginSdkV4.init();
    loginSdkV4.imgLog("//pan.baidu.com/api/analytics", {
        type: "web_header_title_web_login_show",
        clienttype: 0,
        version: "v5",
        currentUrl: window.location.href
    })
